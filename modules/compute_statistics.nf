...
process gc_bias {
    ...
    publishDir "${params.outputDir}/stats/gc_bias", mode: params.enable_neb_agg ? 'copy' : 'link'
...
process idx_stats {
    ...
    publishDir "${params.outputDir}/stats/idxstats", mode: params.enable_neb_agg ? 'copy' : 'link'
...
process flag_stats {
    ...
    tag { library }
    conda "bioconda::samtools=1.9"
    publishDir "${params.outputDir}/stats/flagstats", mode: params.enable_neb_agg ? 'copy' : 'link'
...
process fastqc {
    ...
    publishDir "${params.outputDir}/stats/fastqc", mode: params.enable_neb_agg ? 'copy' : 'link'
...
process insert_size_metrics {
    ...
    tag { library }
    conda "bioconda::picard=3.3.0 bioconda::samtools=1.21"
    publishDir "${params.outputDir}/stats/insert_size", mode: params.enable_neb_agg ? 'copy' : 'link'
...
process picard_metrics {
    ...
    tag { library }
    conda "bioconda::picard=3.3.0 bioconda::samtools=1.21"
    publishDir "${params.outputDir}/stats/picard_alignment_metrics", mode: params.enable_neb_agg ? 'copy' : 'link'
...
process tasmanian {
    label 'medium_cpu'
    tag { library }
    publishDir "${params.outputDir}/stats/tasmanian", mode: params.enable_neb_agg ? 'copy' : 'link'
